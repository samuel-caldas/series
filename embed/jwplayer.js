...
(function(jwplayer) {
	...
	jwplayer.api.events = {
		...
		//A.R
		JWPLAYER_SHOW_HD_LIST: 'jwplayerShowHdList',
		JWPLAYER_HIDE_HD_LIST: 'jwplayerHideHdList'
	};
...
(function(jwplayer) {
	...
	jwplayer.embed = function(playerApi) {
		...
		//A.R set default file
		for(var i = 0; i< _config.hd.length; i++) {
			if(_config.hd[i].label === _config.hd_default) {
				_config.levels = [ { file: _config.hd[i].file } ];
				break;
			}
		}
		//
	...
	jwplayer.html5.controlbar = function(api, config) {
		...
		function _buildElement(element, alignment, parent) {
			...
			switch (element.name) {
				...
				//A.R
				case "hd":
					_addElement("hdText", alignment, true, parent, null, null, _api.skin.getSkinElement("controlbar", "blank"));
					_buildHandler("hdText", "jwShowHDList");
					break;
				//
			}
		}
...
//					wid = 14 + 3 * _settings.fontsize;
		...
		function _setup() {
			...
			_ready = true;
			//A.R
			_elements.hdText.innerHTML = jwplayer(_api.id).config.hd_default;
			_elements.hdText.style.backgroundImage = "url('"+_getBack().src+"')";
			_elements.hdText.style.width = "28px";
			//
			_addListeners();
...
(function(jwplayer) {
	...
	jwplayer.html5.dock = function(api, config) {
		...
		//A.R
		var _hdList = document.createElement("div");
		_hdList.id = api.id + "_jwplayer_hd_list";
		
		_hdList.style.position = "absolute";
		_hdList.style.cursor = "pointer";
		
		_hdList.style.fontWeight = "bold";
		_hdList.style.fontSize = "10px";
		_hdList.style.lineHeight = "20px";
		_hdList.style.fontFamily = "Arial, sans-serif";
		
		_hdList.onmouseover = function(event) {
			clearTimeout(_timeoutID);
		};
		_hdList.onmouseout = function(event) {
			...
			_hdList.style.display = "none";
		};
		
		var _backgroundBar = api.skin.getSkinElement("controlbar", "background");
		
		var _img  = document.createElement("img");
		_img.src = _backgroundBar.src;
		...
		_hdList.appendChild(_img);
		
		var _hdItem;
		
		function _setupHDList() {
			
			var hd = jwplayer(api.id).config.hd;
			var hdDiv, hdLabel;
			
			for(var i = 0; i < hd.length; i++) {
				hdDiv = document.createElement("div");
				hdDiv.style.zIndex = i;
				hdDiv.style.width = "38px";
				hdDiv.style.textAlign = "center";
				hdDiv.onclick = _selectHDFile;
				
				if(hd[i].label === jwplayer(api.id).config.hd_default) {
					hdLabel = document.createTextNode("• "+hd[i].label);
					_hdItem = hdDiv;
				} else {
					hdLabel = document.createTextNode(hd[i].label);
				}
				
				hdDiv.setAttribute("data-label", hd[i].label);
				hdDiv.appendChild(hdLabel);
				_hdList.appendChild(hdDiv);
			}
			_dock.appendChild(_hdList);
		}
		
		function _selectHDFile(event) {
			_hideHDList();
			
			if(_hdItem === event.target) {
				return;
			}
			
			var hdLabel = event.target.innerHTML;
			var hd = jwplayer(api.id).config.hd;
			var hdButton = document.getElementById(api.id+"_jwplayer_controlbar_hdText");
			
			_hdItem.innerHTML = _hdItem.getAttribute("data-label");
			_hdItem = event.target;
			
			for(var i = 0; i < hd.length; i++) {
				if(hd[i].label === hdLabel) {
					hdButton.innerHTML = hdLabel;
					_hdItem.innerHTML = "• "+hdLabel;
					
					var item = jwplayer(api.id).getPlaylist()[0];
					item.levels[0].file = hd[i].file;
					jwplayer(api.id).load([ item ]);
					break;
				}
			}
		}
		...
		var _timeoutID;
		this.showHDList = function() {
			_hdList.style.display = "block";
			
			var hdButton = document.getElementById(api.id+"_jwplayer_controlbar_hdText");
			if(!hdButton.onmouseout) {
				hdButton.onmouseout = function(event) {
					var obj=event.relatedTarget;
					while(obj!=null){
						if(obj==this){
							return;
					...
					_timeoutID = setTimeout(_hideHDList, 50);
				};
			};
			var dx = hdButton.offsetLeft;
			var dy = api.jwGetHeight() - _hdList.offsetHeight - _backgroundBar.height;
			
			_hdList.style.left = dx+"px";
			_hdList.style.top = dy+"px";
		};
		function _hideHDList() {
			_hdList.style.display = "none";
		};
		_setupHDList();
		_hideHDList();
...
(function(jwplayer) {
	...
	jwplayer.html5.api = function(container, options) {
		...
		//A.R
		_api.jwShowHDList = _componentCommandFactory("dock", "showHDList");
		//
